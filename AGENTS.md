...
- All visible statistic values for population, area, GDP, and GDP per capita must include a parenthesized scope/year label such as `（国2025）`, `（県2022）`, `（都市圏2025）`, or `（市域2024）`. GDP must be shown in `兆円`, GDP per capita in `万円`; do not leave GDP cards in dollars, euros, local-currency units, or unlabeled yen values.
...
- Headings inside `.feature-visual`, especially `figcaption strong`, must use `だ・である` plain/nominal Japanese style rather than polite `です・ます` endings. Keep body prose in `です・ます`, but image-overlay section headings should read like concise plain-style editorial headlines.
...
- All city prose should use polite Japanese `です・ます` style. Existing and future uncommitted city pages should be checked and revised away from plain explanatory endings such as `だ`, `である`, and bare dictionary-form sentence endings.
...
- Avoid deliberately adding embedded text, lettering, labels, logos, captions, or readable signs to generated images. Incidental small lettering in otherwise good images is acceptable. Section titles and labels must be HTML overlay text, not pixels inside the image.
- Do not add mosaic, blur, paint-over blocks, or other post-processing solely to hide incidental text in generated images. Some incidental lettering is acceptable. Previously mosaic/paint-over processed published images should be restored or regenerated without that treatment when found.
